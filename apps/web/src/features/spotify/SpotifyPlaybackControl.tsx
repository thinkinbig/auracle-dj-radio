...
function resolveStatusLabel(spotify: SpotifyPlaybackState): string {
  ...
  if (spotify.queueStatus === 'loading') return 'Building from saved/top tracks';
  if (spotify.queueError) return spotify.queueError;
  if (spotify.queueTracks.length > 0 && spotify.enabled) return 'Using saved/top tracks';
  if (spotify.lastMatch && spotify.enabled) return `Spotify Search fallback: ${spotify.lastMatch}`;
