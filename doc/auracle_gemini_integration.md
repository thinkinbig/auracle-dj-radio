...
| Step 1 embed | 跳过 Gemini | `HashEmbedder`（须与 SQLite 索引同空间；Demo 默认 hash） |
...
**模块布局**（`apps/api/src/gemini/`）：

| 模块 | 职责 |
|------|------|
| `circuit-breaker.ts` | FSM（closed / open / half_open） |
| `guard.ts` | 进程级 singleton + dial/stream 记账 |
| `resilience.ts` | `executeWithGeminiFallback` — 唯一 fallback 控制流 |
| `adapters.ts` | `withFlowFallback` / `withEmbedFallback` — 在 seam 上挂 adapter |
| `wiring.ts` | composition root：`buildFlowModel` / `buildEmbedder` |
| `client.ts` | 共享 `GoogleGenAI` 构造 |
...
# Circuit breaker — Flow→heuristic, embed→hash, Live fast-reject (rt_llm_proxy modelcb)
