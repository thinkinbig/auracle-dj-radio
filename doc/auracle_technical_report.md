...
The ranking pipeline centers on a structured energy model. A session has an intended mood and scene; the planner retrieves candidates that match this intent, then orders tracks according to a target energy arc and adjacency penalties. This keeps the session explainable: if a track is chosen, the reason can be traced to explicit metadata rather than opaque generative selection.

When the listener has gathered Spotify seeds (condition C, Premium playback), those seeds replace the catalog as the candidate pool entirely — the same energy-arc sequencing runs over them, but local and Spotify tracks are never ranked into one shared pool. Because Spotify no longer reliably exposes audio features for new apps, Auracle treats Spotify energy as an inferred or reused value. If a Spotify track matches a local catalog track by title and artist, the catalog energy and authored voicing can be reused. Otherwise, best-effort inference and copywriting can enrich the candidate asynchronously after the first playback path.

The important design choice is that Gemini Live does not directly choose tracks during the live session. It can express user intent through tools, but queue construction remains server-controlled and deterministic.
