...
like/dislike 闭环（#68/#69）：后台经 memory-service `POST /taste/session-feedback` 推导 track+artist 偏好 —— Condition C 且已登录时落库（`source: "session"`）并写 mem0 镜像；B/C 下该 session 内的 nudge replan（dislike 带 re-roll）会立即调整接下来 1–2 首。Condition A 与匿名用户不落库；A 队列不变。
