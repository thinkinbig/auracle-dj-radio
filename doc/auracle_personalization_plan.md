...
| ID | 任务 | 涉及文件 | 完成标准 |
|----|------|----------|----------|
| P0-1 | Session 绑定 `user_id` | `session/store.ts`, `server.ts`, `agent-harness/*` | 创建 session 时：有 Bearer → `auth.getUserByToken`；无 → `auracle_anonymous` |
| P0-2 | mem0 per-user | `memory/client.ts` | `MemoryClient` 方法签名加 `userId`；`search`/`add` 用传入 id，不再硬编码 |
| P0-3 | skip 按 user 聚合 | `events-db.ts`, `server.ts` | `skipRateByEnergy(userId, recentSessions)`；SQL JOIN session 元数据或 events 带 `user_id` |
| P0-4 | skip 仅 C 生效 | `server.ts`, `agent-harness/harness/agent-harness.ts`, `replan.ts` | `condition !== "C"` 时 `energyWeights = undefined` |
| P0-5 | replan 传 memories | `session/replan.ts`, `music-engine/flow/plan.ts`, `server.ts` | `applyReplan` 传 `state.mem0Context`（或 fresh recall）；删除 `replan()` 内 `memories: ""` |
| P0-6 | replan 传 energyWeights | 同上 | C 条件 replan 与初始 plan 使用同一 `skipRateByEnergy` |
| P0-7 | 评估环境强制登录 | `apps/web`, 实验 SOP | 被试账号预注册；session 创建带 token；文档写明禁止共用浏览器 profile |

**P0 验收测试（自动化）**

- [ ] 用户 A、B 各 skip 高能量曲 → 仅 A 的 C session 检索降权高能量
- [ ] Condition B 的 `skipRateByEnergy` 调用结果为 `{}` 或不传入 music-engine
- [ ] C replan 请求体含非空 `memories`；B replan 不含 / 为空
- [ ] `record_preference` 写入后，同 user recall 可见；异 user 不可见
