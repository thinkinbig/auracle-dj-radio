...
> **状态 2026-06-25**：P0-1…P0-7 **已实现**（branch `refactor/agent-harness-split-and-review-hardening`，未提交）。
> 编排已迁出 memory-service → 新 **`agent-harness`** 服务（session store / replan / tool-runner / dj-prompt），下表"涉及文件"路径已更新。`ANONYMOUS_USER_ID` 从 `@auracle/shared` 导出。

| ID | 任务 | 涉及文件 | 完成标准 |
|----|------|----------|----------|
| ✅ P0-1 | Session 绑定 `user_id` | `agent-harness/{server.ts,harness/agent-harness.ts,session/store.ts,memory-service-client.ts}` | `POST /sessions` 读 Bearer → `resolveSessionUser`；无 token → `auracle_anonymous`；无效 token → 401。web `createSession` 带 `Authorization` |
...
| ✅ P0-4 | skip 仅 C 生效 | `agent-harness/harness/agent-harness.ts`, `session/replan.ts` | `condition !== "C"` 时 `energyWeights = undefined`、不调 `skipRateByEnergy`、`memories = ""` |
| ✅ P0-5 | replan 传 memories | `agent-harness/session/replan.ts`, `music-engine/flow/plan.ts`, `server.ts` | `applyReplan` 传 C-only `state.mem0Context`；`replan()` 加 `ReplanInput.memories`，删除硬编码 `memories: ""` |
| ✅ P0-6 | replan 传 energyWeights | 同上 + `session/store.ts` | 权重存于 `SessionState.energyWeights`，replan 复用初始 plan 同一份 |
...
**P0 验收测试（自动化）** — `pnpm -r test` 全绿（agent-harness 9 / music-engine 7 / memory-service 5）
...
| ✅ P1-3 | 后端写入规则 | `agent-harness/harness/agent-harness.ts` | C only：同 energy 连续快速 skip ≥2 → 写一条高信号 mem0 事实；避免单次 skip 噪声 |
