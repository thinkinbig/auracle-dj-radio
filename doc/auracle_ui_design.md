...
| 这档 live show 在播 | StageHeader **ON AIR** pill（会话 start 后常亮） |

**辨识度公式**：高对比明暗分区 + 波形主线 + 点阵复古点缀 + **红色 ON AIR = 会话在播** + **绿色 = DJ turn** + 字幕三层状态。
...
| `--accent-live` | `#22C55E` | DJ turn 绿点（Speaking / Listening） |
| `--on-air-bg` | `#DC2626` | ON AIR pill 背景 |
| `--on-air-fg` | `#FFFFFF` | ON AIR pill 文字 |
...
- 右上角：**ON AIR** pill（会话级）+ 会话 elapsed 计时
- 状态行：绿点（turn 级）+ `Speaking…` / `Listening…` / `Playing…`

**两层广播语义**

| 层级 | 含义 | 控件 | 何时亮 |
|------|------|------|--------|
| **Session** | 这档 live show 在播 | 红色 `ON AIR` pill | `phase !== idle`；`paused` 时 dim（opacity ~50%） |
| **Turn** | DJ / 听众正在说话 | 绿色圆点 + pulse | `speaking` / `listening` only |

Play/Pause 按钮保持中性色（transport），不与 ON AIR / Live turn 抢 accent。

**状态与颜色**

| UI 状态 | 触发 | ON AIR | 绿点 | 文案 |
|---------|------|--------|------|------|
| Curating | 用户 tap Start，`POST /sessions` 进行中 | 亮 | 亮 | Tuning in… |
| Speaking | `phase: dj_turn_start` 且收到 model PCM | 亮 | 亮 + pulse | Speaking… |
| Listening | `user_barge_in` 或 mic 活跃 | 亮 | 亮 | Listening… |
| Playing | 音乐 gain 主导、非 DJ turn | 亮 | 灭 | Playing… |
| Paused | 用户本地暂停 | dim | 灭 | Paused |
| Idle | 未 start / 已结束 | 隐藏 | 灭 | Tap to start |

**StageWaveform**

- 24–40 根竖条，宽 2–4px，gap 2–3px
- 高度由 master bus 上的 `AnalyserNode` 驱动（`musicGain` + `djGain` → `masterGain` → analyser），更新用 `transform: scaleY()`，避免改 `height` 引发布局抖动
- **禁止**随机 mock 动画；无 PCM / 无 analyser 时竖条回落到 idle 底高
- DJ turn（`speaking` / `listening`）：白色条（`.live`）
- Playing：灰色条（非 `.live`）
- Curating / idle / paused：**不驱动** analyser，竖条 settle 到 idle 底高（session 计划生成中尚无音频）
...
| curating | 否 | idle |
| playing | 是（曲库 PCM） | 灰 |
| speaking / listening | 是（DJ PCM 或 duck 中的曲库） | 白 |
| paused | 否 | idle |
...
| 多 accent 色乱用 | 破坏「红 = ON AIR、绿 = DJ turn」分层 |
| Play/Pause 用红绿 | 与 ON AIR / 错误态抢语义 |
| 两个 `<audio>` 硬切 | 见音频文档 |
...
| 2026-06-06 | StageWaveform 接入 master bus `AnalyserNode`；移除 mock 随机动画；`curating` 阶段波形 idle |
