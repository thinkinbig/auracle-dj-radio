...
type Config struct {
	...
	// OpeningCue, when non-empty, is sent as the first turn once the Live setup
	// completes, so the model performs the session opening with no client round trip.
	OpeningCue string
}
...
func (g *Gemini) readLoop() {
	...
	for {
		...
		if msg.SetupComplete != nil && g.openingCue != "" {
			cue := g.openingCue
			g.openingCue = "" // send the opening exactly once
			if err := g.SendText(cue); err != nil {
				log.Printf("gemini: opening cue send: %v", err)
			}
		}
