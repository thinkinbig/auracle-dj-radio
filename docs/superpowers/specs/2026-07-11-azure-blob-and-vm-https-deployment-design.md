...
FQDN: auracle-demo.germanywestcentral.cloudapp.azure.com
```

## 工作流 A — 歌单迁 Blob

### A1. 建资源
- StorageV2 / `Standard_LRS` 存储账户(germanywestcentral)。
...
### B2. Caddyfile
```
auracle-demo.germanywestcentral.cloudapp.azure.com {
...
- 公网 IP 配 DNS name label `auracle-demo` → FQDN `auracle-demo.germanywestcentral.cloudapp.azure.com`。
- NSG 入站放行:
  - `22/tcp`(SSH,限操作者 IP)
